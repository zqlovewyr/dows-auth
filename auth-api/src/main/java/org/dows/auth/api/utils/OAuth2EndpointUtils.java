...
/*        auth = request.getHeader("Authorization");
        if (StrUtil.isNotBlank(auth) && auth.indexOf("Basic") == 0) {
            auth = getFromBASE64(auth.substring(6, request.getHeader("Authorization").length()));
            String[] basicAuth = auth.split(":");
            parameters.add("username", basicAuth[0]);
            parameters.add("password", basicAuth[1]);
        }*/
        return parameters;
    }
